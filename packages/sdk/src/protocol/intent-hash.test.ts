/**
 * Cross-language conformance: this TypeScript intent hash must match the
 * Rust side at
 * `zally-core::intent_hash::tests::conformance_vector_zcash_testnet_minimal`.
 *
 * If this test fails, either the byte layout changed on one side and not
 * the other (wire break) or a length prefix, endianness, or domain
 * separator string changed. Fix path: align both sides, regenerate the
 * expected digest, update BOTH conformance vectors in lockstep.
 */
