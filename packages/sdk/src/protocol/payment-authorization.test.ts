...
describe("payment-authorization", () => {
	...
	it("networkToChainReference maps documented vocabulary", () => {
		expect(networkToChainReference("mainnet")).toBe("main");
		expect(networkToChainReference("testnet")).toBe("test");
		expect(networkToChainReference("regtest")).toBe("regtest");
	});

	it("networkToChainReference rejects unknown values", () => {
		expect(() => networkToChainReference("devnet")).toThrow();
	});

	it("paymentUriToCaip10 strips the scheme and any query string", () => {
		expect(paymentUriToCaip10("zcash:utest1qq0?amount=1", "test")).toBe(
			"zcash:test:utest1qq0",
		);
		expect(paymentUriToCaip10("zcash:utest1qq0", "test")).toBe(
			"zcash:test:utest1qq0",
		);
	});

	it("paymentUriToCaip10 rejects a missing scheme", () => {
		expect(() => paymentUriToCaip10("utest1qq0", "test")).toThrow();
	});

	it("paymentUriToCaip10 rejects an empty address", () => {
		expect(() => paymentUriToCaip10("zcash:?amount=1", "test")).toThrow();
	});
});
