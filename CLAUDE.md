...
**`{domain}-{concern}.ts` naming.** File names communicate both domain and concern: `password.ts` (policy + breach check), `well-known.ts` (issuer + discovery metadata), `agent-schemas.ts` (configuration + registration schemas). Predictable for grep, IDE search, and AI agents.

**Sub-directory threshold: 4+ files.** Only create a sub-directory when a concern group has 4+ files after consolidation. Two-file directories are shallow modules at the directory level. Examples: `auth/oidc/jwt/` (5 files, key management), `auth/oidc/haip/` (7 files, HAIP spec).

**Component co-location.** Route-scoped components live in `_components/` next to their page, not in `src/components/`. Only genuinely cross-route components (tier-badge, vault-unlock, ciba-approve-client) belong in `src/components/`.

**Schema by bounded context.** Schema files group by domain boundary: `auth.ts`, `oauth-provider.ts`, `identity.ts`, `privacy.ts` (was crypto.ts), `agent.ts`, `ciba.ts`, `recovery.ts`, `organization.ts`, `oidc-credentials.ts`.
...
See `docs/plans/prd-15-deep-module-reorganization.md` for the full architectural decision record.
