...
Previously, a single `vc:identity` scope dumped all 13 verification status flags to any RP that requested it. A wine shop verifying age received the same data as a bank doing full KYC. This violated the principle of minimal disclosure and gave users no control over which verification details were shared.

The `identity.*` scopes (identity.name, identity.dob, etc.) already supported granular consent, but `vc:identity` had no sub-scope granularity.
...
Chosen option: two-layer selective disclosure using granular `vc:*` sub-scopes with opt-in consent.

### Scope Architecture

**VC verification scopes** (non-PII — derived booleans):

| Scope | Claims | Purpose |
|-------|--------|---------|
| `vc:identity` | All below (umbrella) | Full verification status |
| `vc:verification` | `verification_level`, `verified` | Basic "is verified" check |
| `vc:age` | `age_proof_verified` | Age-gated services |
| `vc:document` | `document_verified`, `doc_validity_proof_verified` | Document verification |
| `vc:liveness` | `liveness_verified`, `face_match_verified` | Biometric verification |
| `vc:nationality` | `nationality_proof_verified` | Nationality proof |
| `vc:compliance` | `policy_version`, `issuer_id`, `verification_time`, `attestation_expires_at` | Audit metadata |
...
1. RP requests `vc:identity` (umbrella scope)
2. Consent page **expands** `vc:identity` into individual `vc:*` sub-scope checkboxes
3. All checkboxes start **unchecked** (opt-in, not opt-out)
4. User selects only the claims they want to share
5. Approved scopes (e.g., `vc:verification vc:age`) go into the access token
...
* A wine shop requesting `vc:identity` only gets `verified` + `age_proof_verified` if that's all the user approves
* A bank gets full verification status + PII because the user opts in to everything
* DCR-registered clients request `vc:identity` (allowed in `publicClientScopes`) and the user controls the rest
...
* **Server-side scope enforcement only**: RP pre-defines granular scopes (`vc:age`), no user choice at consent. Rejected because this is access control, not selective disclosure — the user has no agency.
* **Single `vc:identity` scope with all-or-nothing consent**: The previous approach. Rejected because it violates minimal disclosure.
* **Merge vc:* and identity.* into one family**: Would lose the clear separation between non-PII verification status and actual personal data, which serve fundamentally different use cases.

## More Information

* VC scope implementation: `apps/web/src/lib/auth/oidc/vc-scopes.ts` (scope mapping and filtering)
