...
- **Stable nullifier scope**: The ZKPassport `scope` is stable for Zentity's NFC identity-verification flow. This keeps `chipNullifier` stable for duplicate detection. Freshness and replay protection use session-bound `custom_data`, not a randomized scope.
- **One-time proof session binding**: The server creates an `identity_verification_sessions` row before launching the mobile request. The row stores the expected query profile hash, stable proof scope, one-time proof binding, expiry, and consumption state. The submitted ZKPassport result must include `bind.custom_data` equal to that proof binding. Missing, expired, consumed, wrong-profile, or wrong-binding sessions are rejected before proof verification.
...
### Credential Binding Commitment Admission
