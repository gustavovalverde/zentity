...
use crate::frost::types::{
    ...
    ParticipantId, Session, SessionId, SignerCommitRequest, SignerCommitResponse,
    SignerDkgFinalizeRequest, SignerDkgFinalizeResponse, SignerDkgRound1Request,
    SignerDkgRound1Response, SignerDkgRound2Request, SignerDkgRound2Response,
    SignerPartialSignRequest, SignerPartialSignResponse, SigningAggregateRequest,
    SigningAggregateResponse, SigningCommitRequest, SigningCommitResponse, SigningInitRequest,
    SigningInitResponse, SigningSession, SigningState, SigningSubmitPartialRequest,
    SigningSubmitPartialResponse,
};
...
impl Coordinator {
    ...
    pub async fn init_dkg(&self, request: DkgInitRequest) -> SignerResult<DkgInitResponse> {
        ...
        let participant_endpoints = if let Some(endpoints) = request.participant_endpoints {
            ...
            for (i, endpoint) in self.signer_endpoints.iter().enumerate() {
                // Safety: participant count is validated < u16::MAX, i+1 is always > 0
                #[allow(clippy::cast_possible_truncation)]
                let raw_id = (i + 1) as u16;
                if raw_id <= request.total_participants {
                    let participant_id = ParticipantId::new_unwrap(raw_id);
    ...
    pub async fn init_signing(
        ...
        #[allow(clippy::cast_possible_truncation)]
        let selected_signers = request.selected_signers.unwrap_or_else(|| {
            (1..=self.signer_endpoints.len() as u16)
                .filter_map(ParticipantId::new)
                .collect()
        });

        let mut signer_endpoints: HashMap<ParticipantId, String> = HashMap::new();
        for (i, endpoint) in self.signer_endpoints.iter().enumerate() {
            // Safety: participant count is typically small (< 100), i+1 > 0
            #[allow(clippy::cast_possible_truncation)]
            let participant_id = ParticipantId::new_unwrap((i + 1) as u16);
        ...
        if selected_signers
            .iter()
            .any(|id| id.get() > group_key.total_participants)
        {
    ...
    pub async fn aggregate_signatures(
        ...
        let signature_hex = match session.ciphersuite {
            crate::config::Ciphersuite::Secp256k1 => self.aggregate_secp(&session, &message)?,
            crate::config::Ciphersuite::Ed25519 => self.aggregate_ed(&session, &message)?,
        };
    ...
    /// Aggregate secp256k1 signature shares into final signature.
    #[allow(clippy::unused_self)]
    fn aggregate_secp(&self, session: &SigningSession, message: &[u8]) -> SignerResult<String> {
        let pubkey_package = decoders::decode_pubkey_package_secp(&session.public_key_package)?;
        let commitments = decoders::decode_commitments_secp(&session.commitments)?;
        let shares = decoders::decode_signature_shares_secp(&session.partial_signatures)?;

        let signing_package = frost_secp::SigningPackage::new(commitments, message);
        let signature = frost_secp::aggregate(&signing_package, &shares, &pubkey_package)
            .map_err(|e| decoders::extract_culprit_from_secp_error(&e))?;
    ...
    /// Aggregate Ed25519 signature shares into final signature.
    #[allow(clippy::unused_self)]
    fn aggregate_ed(&self, session: &SigningSession, message: &[u8]) -> SignerResult<String> {
        let pubkey_package = decoders::decode_pubkey_package_ed(&session.public_key_package)?;
        let commitments = decoders::decode_commitments_ed(&session.commitments)?;
        let shares = decoders::decode_signature_shares_ed(&session.partial_signatures)?;

        let signing_package = frost_ed::SigningPackage::new(commitments, message);
        let signature = frost_ed::aggregate(&signing_package, &shares, &pubkey_package)
            .map_err(|e| decoders::extract_culprit_from_ed_error(&e))?;
