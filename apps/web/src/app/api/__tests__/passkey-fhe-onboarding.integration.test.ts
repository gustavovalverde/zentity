...
describe("passkey onboarding + FHE finalize integration", () => {
  let blobDir = "";
  ...
  it("finalizes FHE enrollment after pre-auth blob upload", async () => {
    ...
    const { contextToken, registrationToken } = await createOnboardingContext({
      userId,
    ...
    await expect(getOnboardingContext(contextToken)).resolves.toBeNull();
