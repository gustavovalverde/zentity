...
describe("jwt-signer dual-algorithm dispatcher", () => {
  let edDsaKid: string;
  ...
  describe("id tokens (no scope)", () => {
    it("signs with EdDSA by default", async () => {
      const token = await signJwt({
        aud: "some-client-id",
        sub: "user-1",
        iss: "https://zentity.test",
  ...
  describe("resolveClientId extraction", () => {
    it("extracts from string aud", async () => {
      ...
      // Token signed successfully means clientId was resolved (or defaulted to EdDSA)
    ...
    it("defaults to EdDSA when no clientId is resolvable", async () => {
      const token = await signJwt({
        sub: "user-1",
      });
