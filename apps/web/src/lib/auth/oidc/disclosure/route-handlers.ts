...
/**
 * HTTP route handlers for the identity disclosure pipeline.
 *
 * These thin wrappers around `delivery.ts` add the auth + rate-limit + body
 * parse boilerplate. Kept separate from delivery.ts so the pure delivery
 * primitives don't pull in `resource-auth` (which would create an import
 * cycle: auth-config → disclosure → resource-auth → auth-config).
 *
 * Used by /api/oauth2/identity/{intent,stage,unstage} and
 * /api/ciba/identity/{intent,stage,unstage}.
 */
