...
const contextIdentifier = (token: string) => `onboarding:context:${token}`;
...
export async function createOnboardingContext(params: {
  userId: string;
  email?: string | null;
}): Promise<{
  contextToken: string;
...
export async function getOnboardingContext(
  contextToken: string
): Promise<OnboardingContextValue | null> {
  ...
  try {
    return JSON.parse(record.value) as OnboardingContextValue;
  } catch {
    return null;
  }
}
...
export async function consumeOnboardingContext(contextToken: string) {
