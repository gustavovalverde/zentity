...
describe("authorization_code id_token claims filtering", () => {
  ...
  it("applies claims.id_token filters during a live authorization_code exchange", async () => {
    ...
    expect(idTokenClaims.amr).toEqual(["pwd"]);
    expect(idTokenClaims.acr_eidas).toBe("http://eidas.europa.eu/LoA/low");
    expect(idTokenClaims.zentity_assurance).toBeUndefined();
