...
/**
 * Check if a specific device matches the saved preference.
 */
export function isPreferredDevice(
  device: MediaDeviceInfo,
  preference: StoredCameraPreference | null
): boolean {
  if (!preference) {
    return false;
  }

  return device.deviceId === preference.deviceId;
}
