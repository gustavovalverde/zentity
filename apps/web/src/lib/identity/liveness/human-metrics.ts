...
function _getPitchDegrees(face: HumanFaceResult | null): number {
  const pitchRad = face?.rotation?.angle?.pitch;
  if (typeof pitchRad === "number") {
...
function _getRollDegrees(face: HumanFaceResult | null): number {
  const rollRad = face?.rotation?.angle?.roll;
  if (typeof rollRad === "number") {
