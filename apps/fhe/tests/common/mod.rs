...
/// - `CompressedPublicKey`: Public key for encryption
...
pub fn get_test_keys() -> (ClientKey, CompressedPublicKey, String) {
    ...
    // Register server + public key once and cache the key_id
...
pub fn get_public_key() -> CompressedPublicKey {
    let material = init_test_keys();
...
pub fn get_registered_key_id() -> String {
    let (_, _, key_id) = get_test_keys();
    key_id
}
